...
src/
  ...
  settings/        # Settings UI (API keys, providers, sync)
  viewer/          # Article viewer page
  background/      # Service worker (orchestration + sync)
  shared/          # Services: ai, image, storage, auth, onedrive, sync, crypto, settings, blob-storage
```
Recipes, AI provider calls, image provider calls, and shared types live in `@kypflug/transmogrifier-core` — not in this repo.
...
- Two-tier encryption: device key (IndexedDB) for local, identity key (HKDF from Microsoft user ID) for OneDrive sync
- Identity key: HKDF-SHA256 with fixed salt "transmogrifier-settings-v2" and info "settings-encryption"
- Legacy v1 passphrase envelopes supported for migration (PBKDF2 600k)
...
- 6 recipes (defined in core): Reader (default), Aesthetic, Illustrated, Visualize, Interview, Custom
